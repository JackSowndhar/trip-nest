...
router.post('/', protect, async (req, res) => {
  try {
    ...
    if (userToAdd._id.toString() === req.user._id.toString()) {
      return res.status(400).json({ message: 'You cannot invite yourself to this trip' });
    }
    ...
    const existingInvite = await Invitation.findOne({ trip: tripId, invitee: userToAdd._id, status: 'pending' });
    if (existingInvite) {
      return res.status(400).json({ message: 'An invitation is already pending for this user' });
    }

    // Create invitation instead of member
