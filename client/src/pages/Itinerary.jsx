...
import { Target, Utensils, Car, Hotel, Moon, MapPin, Calendar, Clock, ArrowLeft, Pencil, Trash2, X, Check } from 'lucide-react';

const TYPES = [
  { value: 'activity', label: 'Activity', icon: Target, color: 'bg-green-100 text-green-700 border-green-200' },
  { value: 'food', label: 'Food', icon: Utensils, color: 'bg-orange-100 text-orange-700 border-orange-200' },
  { value: 'transport', label: 'Transport', icon: Car, color: 'bg-purple-100 text-purple-700 border-purple-200' },
  { value: 'stay', label: 'Stay', icon: Hotel, color: 'bg-blue-100 text-blue-700 border-blue-200' },
  { value: 'rest', label: 'Rest', icon: Moon, color: 'bg-gray-100 text-gray-600 border-gray-200' },
  { value: 'other', label: 'Other', icon: MapPin, color: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
];
          ...
          {TYPES.map(t => {
            ...
            return (
              <button key={t.value} type="button" onClick={() => setForm(p => ({ ...p, type: t.value }))}
                className={`py-2 px-2 rounded-xl text-xs font-medium transition-all text-center flex items-center justify-center gap-1.5
...
export default function Itinerary() {
  ...
  const handleEdit = async (data) => {
    ...
    try {
      ...
      toast.success(`Activity "${data.title}" updated!`);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(`Activity "${deleteConfirm.title}" deleted.`);
    } catch (err) {
      console.error(err);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-6">
          ...
          <div className="lg:col-span-3">
            {!activeDayData || activeDayData.items.length === 0 ? (
              <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-8 sm:p-14 text-center flex flex-col items-center justify-center">
                      ...
                      <div className="flex items-start gap-3 w-full">     
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            {item.time && <span className="text-xs font-mono text-gray-400 bg-gray-50 px-2 py-0.5 rounded-lg flex items-center gap-1"><Clock className="w-3 h-3" /> {item.time}</span>}
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full border flex items-center gap-1.5 ${typeInfo.color}`}>
                          ...
                          {item.location && <p className="text-xs text-gray-400 mt-0.5 flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-gray-400" /> {item.location}</p>}
                        ...
                        <div className="flex gap-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex-shrink-0">
                          ...
                          <button onClick={() => setDeleteConfirm(item)}
                            className="p-1 text-xs border border-gray-200 text-gray-400 rounded-lg hover:border-red-300 hover:text-red-500 transition-colors flex items-center justify-center">
