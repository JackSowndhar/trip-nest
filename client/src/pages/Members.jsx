      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
              ...
              return (
                <div key={member._id} className="p-4 sm:p-5 flex flex-col xs:flex-row xs:items-center justify-between gap-3 xs:gap-4">
                  <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                    <div 
                      onClick={() => u.avatar && showPreview(u.avatar, u.name)}
                      className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-gradient-to-br from-primary-500 to-emerald-500 flex items-center justify-center shadow-sm flex-shrink-0 transition-transform ${u.avatar ? 'cursor-zoom-in hover:scale-105' : ''}`}
                    >
                      {u.avatar ? (
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-4 sm:p-6 self-start space-y-4">
          ...
          <p className="text-xs text-gray-400 leading-relaxed">
            Invite friends to join this trip. Once they accept the invitation, they will be able to view details, plan itineraries, and log expenses.
          </p>
                        ...
                        >
                          <div 
                            onClick={(e) => {
                              if (u.avatar) {
                                e.stopPropagation();
                                showPreview(u.avatar, u.name);
                              }
                            }}
                            className={`w-7 h-7 overflow-hidden bg-gradient-to-br from-primary-500 to-emerald-500 rounded-full flex items-center justify-center flex-shrink-0 transition-transform ${u.avatar ? 'cursor-zoom-in hover:scale-105' : ''}`}
                          >
                            {u.avatar ? (
                              <img src={u.avatar} alt={u.name} className="w-full h-full object-cover" />
                            ) : (
