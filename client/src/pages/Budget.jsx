...
import { Utensils, Hotel, Car, Target, ShoppingBag, Package, Calendar, Calculator, Trash2, ArrowLeft, AlertTriangle, CheckCircle2, Circle, Coins } from 'lucide-react';

const CATEGORIES = [
  { value: 'food', label: 'Food', icon: Utensils, color: 'bg-orange-100 text-orange-700' },
  { value: 'stay', label: 'Stay', icon: Hotel, color: 'bg-blue-100 text-blue-700' },
  { value: 'transport', label: 'Transport', icon: Car, color: 'bg-purple-100 text-purple-700' },
  { value: 'activity', label: 'Activity', icon: Target, color: 'bg-green-100 text-green-700' },
              ...
              <button key={m._id} type="button" onClick={() => setForm(p => ({ ...p, paidBy: m._id }))}
                ...
                <span 
                  onClick={(e) => {
                    if (m.avatar) {
                      e.stopPropagation();
                      showPreview(m.avatar, m.name);
                    }
                  }}
                  className={`w-4 h-4 rounded-full overflow-hidden text-white text-[10px] flex items-center justify-center font-bold transition-transform ${m.avatar ? 'cursor-zoom-in hover:scale-110' : ''}`}
            ...
            {members.map(m => {
              ...
              return (
                <button key={m._id} type="button" onClick={() => toggleSplit(m._id)}
                  ...
                  <span>{selected ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Circle className="w-3.5 h-3.5" />}</span>
                  {m.name.split(' ')[0]}
                </button>
              );
            })}
...
setMembers(normalized);
  ...
  const handleEdit = async (form) => {
    try {
      ...
      toast.success(`Expense "${form.title}" updated!`);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success(`Expense "${deleteConfirm.title}" deleted.`);
    } catch (err) {
      console.error(err);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
            ...
            <div className="lg:col-span-2 space-y-3">
              {data.expenses.length === 0 ? (
                <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-8 sm:p-16 text-center flex flex-col items-center justify-center">
                  ...
                  return (
                    <div key={exp._id} className="bg-white rounded-2xl border border-gray-100 p-4 hover:shadow-md transition-all group">
                      <div className="flex items-start gap-3">
                        <span className={`px-2.5 py-1 rounded-xl text-xs font-semibold flex-shrink-0 flex items-center gap-1.5 ${cat.color}`}>
                      ...
                      return (
                        <div key={cat}>
                          <div className="flex justify-between text-xs mb-1">
                            <span className={`font-medium px-2 py-0.5 rounded-full flex items-center gap-1 ${c.color}`}>
                    ...
                    {data.memberSpend.map(({ member, paid, owes }) => {
                      ...
                      return (
                        <div key={member._id} className="flex items-center gap-3">
                          <div 
                            onClick={() => member.avatar && showPreview(member.avatar, member.name)}
                            className={`w-8 h-8 rounded-full overflow-hidden flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-1 transition-transform ${member.avatar ? 'cursor-zoom-in hover:scale-105' : ''}`}
                          ...
                          <span className={`text-xs font-bold mr-3 ${net > 0 ? 'text-green-600' : net < 0 ? 'text-red-500' : 'text-gray-400'}`}>
